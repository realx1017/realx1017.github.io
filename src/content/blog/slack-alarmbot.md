...
### ③ Bedrock Claude 4.5 프롬프팅 철학 (`llm_analyzer.py`)
AI가 피상적인 로그 메시지만 보고 답하는 것을 방지하기 위해 다음과 같은 규칙을 시스템 프롬프트(System Prompt)로 엄격히 주입하였습니다.
- **모델 ID**: `global.anthropic.claude-sonnet-4-5` (Cross-Region Inference Profile 활용)
- **시스템 프롬프트 명세**:
```python
...
2. **Bedrock ValidationException (apac prefix 에러)**
   ...
   - **해결**: 크로스 리전 모델 ID인 `global.anthropic.claude-sonnet-4-5`로 고정하여 가용 영역 간의 자동 모델 가동을 구현.
...
- **AWS Bedrock 비용**: Claude 4.5 입력 2,000 토큰 + 출력 1,200 토큰 기준 ➡️ 약 **$0.01 / 회**
